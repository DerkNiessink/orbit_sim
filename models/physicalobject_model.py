...
class PhysicalObjectModel:
    ...
    force_cache: dict[tuple[PhysicalObjectModel, PhysicalObjectModel], Vector2] = dict()
    ...
    def net_force(self, bodies: Sequence[PhysicalObjectModel]) -> Vector2:
        """Calculate the net force on the body."""
        return sum((self.two_body_force(body) for body in bodies), start=self.null_vector)

    def two_body_force(self, other_body: PhysicalObjectModel) -> Vector2:
    ...
    def calculate_two_body_force(self, other_body: PhysicalObjectModel) -> Vector2:
